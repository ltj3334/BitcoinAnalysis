...
class illegal_analysis():
    ...
    def __get_indexed_address_using_origin_address(self, target_addr_list_file_path):
        ...
        for addr_list in tqdm(slicing_origin_addr, desc = "Origin Addr To Index Addr"):
            ...
            self.addr_id_dict.update({k: g["id"].tolist() for k,g in results.groupby("addr")})
            ...
            for tx in tx_list:
                if int(tx) in self.tx_btc_price_hash.keys():
                    ...
                    if len(sqlresult) == 1: # txin이 1개 일 경우, 해당 주소로 입금된 금액만 확인
                        ...
                        self.curCore.execute(sql)
                        second_sqlresult = self.curCore.fetchall()
                        if second_sqlresult:
                            btc_results = [x[0] for x in second_sqlresult]
                            if len(btc_results) <= 2:
                                result = np.array(btc_results)
                                result *= float(self.tx_btc_price_hash[int(tx)])
                                self.addr_usd_mean_value[addr] = [np.mean(result), np.std(result)]
                        ### 220714 삭제
                        ...
                        if len(result.keys()) > 1: # 다 대 다  구조일 경우, 같이 입금된 애가 여러개임                            
                            if len(result.keys()) < 5:
                                ...
                                if result[addr][0] == max_btc_value:
                                    ...
                                    self.addr_usd_mean_value[addr] = [np.mean(btc_results), np.std(btc_results)]
                                else:
                                    continue
...
for file in os.listdir('./address'):
    if "chainalysis_ransom" in file:
        ...
        for addr, [mean, std] in il.addr_usd_mean_value.items():
            addr = il.id_addr_dict[addr]
            saved_list.append([addr[0], mean, std])
