...
from scipy.stats import entropy, describe, moment
...
class illegal_analysis():
    ...
    def calc_derivation_using_tx_list(self, target_addr_list_file_path):
        ...
        self.target_addr_list_file_path = target_addr_list_file_path
    ...
    def calc_statistic_value(self):
        # self.addr_usd_hash #여기 USD리스트 들어가있음
        stat_dict = []
        for addr, usd_list in self.addr_usd_hash.items():
            
            mean = moment(usd_list, 1)
            variance = moment(usd_list, 2)
            skewness = moment(usd_list, 3)
            kurtosis = moment(usd_list, 4)
            stat_dict.append([addr, mean, variance, skewness, kurtosis])
            # stat_dict.append([addr, moments.mean, moments.variance, moments.skewness, moments.kurtosis])            
        
        saved_df = pd.DataFrame(stat_dict)
        saved_df.columns=['address','mean','variance','skewness','kurtosis']
        target_name = self.target_addr_list_file_path.split("_")
        saved_df.to_csv(f"{target_name[0]}_{target_name[1]}_moments_result.csv", header = False, index = False)
...
########  변수 입력  #############################################################################
core_db_path = "/home/tj/BitcoinAnalysis/dbv3-core.db"
index_db_path ="/home/tj/BitcoinAnalysis/dbv3-index.db"
